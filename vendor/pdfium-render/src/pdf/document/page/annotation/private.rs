pub(crate) mod internal {
    ...
    use crate::bindgen::{
        ...
        FPDF_ANNOT_FLAG_TOGGLENOVIEW, FPDF_OBJECT_STRING, FPDF_WCHAR, FS_RECTF,
    };
        ...
        PdfPageAnnotationCommon + PdfiumLibraryBindingsAccessor<'a>
    {
        ...
        fn fill_color_impl(&self) -> Result<PdfColor, PdfiumError> {
            ...
            if self.bindings().is_true(unsafe {
                ...
                // PATCHED for pdf-annotate (see PATCHES.md). FPDFAnnot_GetColor()
                // returns false when the annotation has an appearance stream. Upstream
                // then passed this annotation handle to FPDFPageObj_GetFillColor() as
                // though it were a page object; Pdfium reads the wrong structure and
                // the process crashes. Report the failure instead.
                Err(PdfiumError::PdfiumLibraryInternalError(
                    PdfiumInternalError::Unknown,
                ))
            }
        ...
        fn set_fill_color_impl(&mut self, fill_color: PdfColor) -> Result<(), PdfiumError> {
            if self.bindings().is_true(unsafe {
                ...
                // PATCHED for pdf-annotate (see PATCHES.md). Upstream passed this
                // annotation handle to FPDFPageObj_SetFillColor() as though it were a
                // page object, which corrupts memory. Report the failure instead.
                Err(PdfiumError::PdfiumLibraryInternalError(
                    PdfiumInternalError::Unknown,
                ))
            }
        ...
        /// Internal implementation of [PdfPageAnnotationCommon::remove_appearance()].
        /// Added for pdf-annotate (see PATCHES.md).
        #[inline]
        fn remove_appearance_impl(
            &mut self,
            mode: crate::pdf::appearance_mode::PdfAppearanceMode,
        ) -> Result<(), PdfiumError> {
            // A null value tells Pdfium to remove the appearance stream for this mode;
            // for the normal mode it removes the whole /AP entry.
            if self.bindings().is_true(unsafe {
                self.bindings()
                    .FPDFAnnot_SetAP(self.handle(), mode.as_pdfium(), std::ptr::null())
            }) {
                Ok(())
            } else {
                Err(PdfiumError::PdfiumLibraryInternalError(
        ...
        fn stroke_color_impl(&self) -> Result<PdfColor, PdfiumError> {
            ...
            if self.bindings().is_true(unsafe {
                ...
                // PATCHED for pdf-annotate (see PATCHES.md). FPDFAnnot_GetColor()
                // returns false when the annotation has an appearance stream. Upstream
                // then passed this annotation handle to FPDFPageObj_GetStrokeColor() as
                // though it were a page object; Pdfium reads the wrong structure and
                // the process crashes. Report the failure instead.
                Err(PdfiumError::PdfiumLibraryInternalError(
                    PdfiumInternalError::Unknown,
        ...
        fn set_stroke_color_impl(&mut self, stroke_color: PdfColor) -> Result<(), PdfiumError> {
            if self.bindings().is_true(unsafe {
                ...
                // PATCHED for pdf-annotate (see PATCHES.md). Upstream passed this
                // annotation handle to FPDFPageObj_SetStrokeColor() as though it were a
                // page object, which corrupts memory. Report the failure instead.
                Err(PdfiumError::PdfiumLibraryInternalError(
                    PdfiumInternalError::Unknown,
                ))
