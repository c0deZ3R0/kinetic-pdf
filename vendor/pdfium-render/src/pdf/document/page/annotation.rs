...
pub trait PdfPageAnnotationCommon {
    ...
    /// Removes this [PdfPageAnnotation]'s appearance stream for the given mode; for
    /// [PdfAppearanceMode::Normal] this removes the whole appearance dictionary. Pdfium
    /// reports an annotation's color entries only when it has no normal appearance
    /// stream, so call this before [PdfPageAnnotationCommon::stroke_color()] or
    /// [PdfPageAnnotationCommon::fill_color()] to read the colors of an annotation that
    /// has one. Added for pdf-annotate (see PATCHES.md).
    ///
    /// [PdfAppearanceMode::Normal]: crate::pdf::appearance_mode::PdfAppearanceMode::Normal
    fn remove_appearance(
        &mut self,
    ...
    T: PdfPageAnnotationPrivate<'a>,
{
    ...
    fn remove_appearance(
        ...
        self.remove_appearance_impl(mode)
    }
