...
impl<'a> PdfPage<'a> {
    ...
    /// Local patch, see PATCHES.md. Renders like [PdfPage::render_into_bitmap_with_config()],
    /// but a step at a time: roughly every `step`, Pdfium pauses and `keep_going` is called
    /// with the bitmap as drawn so far. Returning `false` abandons the render.
    ///
    /// Returns `Ok(true)` once the page is completely drawn, `Ok(false)` if it was abandoned.
    /// Custom transformation matrices and form field highlight colours are not supported;
    /// form fields themselves are drawn once the page is complete.
    pub fn render_into_bitmap_in_steps(
        &self,
        bitmap: &mut PdfBitmap,
        config: &PdfRenderConfig,
        step: std::time::Duration,
        mut keep_going: impl FnMut(&PdfBitmap) -> bool,
    ) -> Result<bool, PdfiumError> {
        struct Deadline(std::cell::Cell<std::time::Instant>);

        unsafe extern "C" fn need_to_pause_now(pause: *mut IFSDK_PAUSE) -> FPDF_BOOL {
            // SAFETY: `user` points at the `Deadline` below, which outlives the render.
            let deadline = unsafe { &*((*pause).user as *const Deadline) };
            FPDF_BOOL::from(std::time::Instant::now() >= deadline.0.get())
        }
        ...
        let deadline = Deadline(std::cell::Cell::new(std::time::Instant::now() + step));
        let mut pause = IFSDK_PAUSE {
            version: 1,
            NeedToPauseNow: Some(need_to_pause_now),
            user: &deadline as *const Deadline as *mut std::os::raw::c_void,
        };
        ...
        while status == FPDF_RENDER_TOBECONTINUED as c_int {
            if !keep_going(bitmap) {
                unsafe { self.bindings().FPDF_RenderPage_Close(self.page_handle) };
                return Ok(false);
            }
            deadline.0.set(std::time::Instant::now() + step);
            status = unsafe { self.bindings().FPDF_RenderPage_Continue(self.page_handle, &mut pause) };
        }
        unsafe { self.bindings().FPDF_RenderPage_Close(self.page_handle) };

        if status != FPDF_RENDER_DONE as c_int {
